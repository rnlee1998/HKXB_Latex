...
\title{
\vspace{0.39cm} \xiaoerhao\textbf{基于图像的小目标检测研究与实现} \xiaowuhao \mycolorBlue{（中文题名不超过20字，英文题名不超过10个实词）}\vspace{0.4cm}
}
...
\author{
\sihao xx \makebox{$^{\text{1, *}}$}\\[0.3cm]
\xiaowuhao 1.~~北京航空航天大学~~自动化科学与电气工程学院，北京~~100191\mycolorBlue{（投稿时请将作者姓名和单位删掉）}
}
...
%      中文摘要
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\CKeyword{红外图像；弱小目标；目标检测； 细粒度图像分类；注意力机制 }
\CLCNo{(请自行查找,至少一个V字开头的)}
\Dcode{A}
\PaperNo{1001-6893 （2022） XX-XXXXXX-XX}
...
\begin{CAbstractAAAS}
红外小目标检测技术在军用、民用领域，都有着十分广泛的应用场景。尽管在技术上取得了长足的进步，但仍充满挑战。为了解决传统方法易产生“高虚警”的问题，构建一个高效可行、适用范围广泛、强鲁棒性的小目标检测系统，需要从小目标特征提取、细粒度分类方面进行考虑，本文主要贡献体现在如下几个方面：
1)	红外目标检测方面，不同于传统方法，本文主要利用深度学习的方法开展对红外小目标检测的研究，涉及到的方法有Faster R-CNN, SSD, Yolo等。
2)	小目标特征提取方面，融合FPN与Faster + FPN，并创新性用于红外数据集，利用网络浅层高分辨率的特点，增强对小目标特征的提取。在红外飞机数据集上，测试了Faster R-CNN+FPN在简单背景和复杂噪声背景下的检测性能，在对比实验中取得了最高的检测精度，相比未加入FPN前的结果有显著提升。
...
\parbox{\textwidth}{%
\makebox[\widthof{\makebox{*}收}][r]{收}稿日期: 2022-xx-xx； 录用日期： 2022-xx-xx； 网络出版时间：2022-xx-xx； \\%
...
\hei\makebox[\widthof{\makebox{*}收}][r]{\makebox{*}通}信作者：E-mail：rnlee1998@buaa.edu.cn
}}
  \end{@twocolumnfalse}
]
...
\section{引言的说明}
《航空学报》一般不在篇首编排符号表，符号请在正文中首次出现时说明。引言不编排节号。引言应说明课题的背景，引述该领域的国内外同行已经取得的进展，以说明本文的选题意义和创新点所在\citeAAAS{Zarchan2013Tactical,NESLINE1981Anew}。 内容不应与摘要和结论雷同。最好不要插图列表\citeAAAS{He2017A}。在论述本文的研究意义时，应注意分寸，切忌使用“有很高学术价值”、“填补了国内外空白”、“首次发现”等不适之词；同时也注意不要使用客套话，如 “才疏学浅”、“水平有限”、“恳求指教”之类的语言\citeAAAS{r1,r2,r3}。

\section{题目、作者和单位}

题目应简洁、准确，能恰如其分地概括研究的范围和深度，避免使用希腊字母和上下标。英文题名中第一个单词首字母大写，其余小写(专有名词首字母大写)\citeAAAS{r5}。

作者署名及署名排序应协商一致。姓名的英译采用汉语拼音，姓前名后，姓全大写，名首字母大写。如：ZHANG Ying（张颖），WANG Xilian（王锡联），ZHUGE Hua（诸葛华）。

通讯作者一般为导师或课题负责人。

单位应为论文首次投稿时的作者所在单位。单位的著录一般应到系一级，单位应著录全称，单位名称的英译应统一正确\citeAAAS{r9}。
...
3) 应在正文中顺次引述（按在正文中被提及的先后来排列各篇参考文献的序号，所有参考文献均应在正文中提及），上标引用\citeAAAS{r2}，应该也可以文献\cite{He2017A}怎么怎么；
...
1.\textbf{\textsl{\ArialFont School of Automation Science and Elecical Engineering, Beihang University, Beijing 100191, China}}}
...
\parbox{\textwidth}{%
\CalibriFont
\makebox[\widthof{\makebox{*}R}][r]{R}eceived: 2022-xx-xx; Revised: 2022-xx-xx; Accepted: 2022-xx-xx; \\%
\makebox[\widthof{\makebox{*}U}][r]{U}RL: http://hkxb.buaa.edu.cn/CN/html/2022XXXX.html\\
...
\makebox[\widthof{\makebox{*}C}][r]{\makebox{*}C}orresponding author. ~~ E-mail: rnlee1998@buaa.edu.cn
}}
  \end{@twocolumnfalse}
]
